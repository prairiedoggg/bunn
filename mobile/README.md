...
## 소셜 로그인(구글/애플/네이버/카카오)

모바일은 브라우저로 OAuth를 진행한 뒤 Rails 콜백에서 JWT를 발급받아 앱으로 돌아옵니다.

- 시작 URL: `${API_BASE_URL}/api/auth/{provider}?return_to=<앱 딥링크>`
- Rails 콜백은 `return_to`로 `#token=...`을 붙여 리다이렉트합니다.

주의: 실제 로그인은 각 제공자 콘솔에 **Redirect URI(= Rails 콜백 URL)** 를 등록하고 클라이언트 키를 넣어야 동작합니다.
